...
export default function AdminMainShopListViewPage() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const viewId = searchParams.get('id');
  const [view, setView] = useState<IShop>({
    new_img_file: null,
    org_file_path: '',
    org_file_name: '',
    title: '',
  ...
  const callView = async () => {
    ...
    try {
      const url = `/api/admin/mgmt/shop?take=999`;
      const result: { code: number; data: any; message: string } =
        await callAPI(HTTPMETHOD.GET, {}, url, token);
      const mgmtList = result.data.mgmt;
      const shopItem = mgmtList.filter(
        (item: any) => item.index.toString() === viewId?.toString(),
      )[0];
      if (shopItem == null || shopItem == undefined) {
        alert('존재하지 않는 SHOP 정보입니다.');
        router.back();
        return;
      }
      const shopItemCopy = {
        new_img_file: null,
        org_file_path: shopItem.files[0] ? shopItem.files[0].file_path : '',
        org_file_name: shopItem.files[0] ? shopItem.files[0].file_name : '',
        title: shopItem.title,
        title_en: shopItem.title_en,
        title_cn: shopItem.title_cn,
        title_cn_tw: shopItem.title_cn_tw,
        link: shopItem.link,
      };
      console.log(shopItemCopy);
      setView(shopItemCopy);
    } catch (e: any) {
      console.log(e);
    }
  };
  ...
  const save = async (event: FormEvent) => {
    ...
    view.new_img_file && formData.append('main_shop_img', view.new_img_file);
    ...
    try {
      ...
      router.push('/admin/pages/main/shoplist');
      // router.back();
    } catch (e: any) {
      alert(
        ...
        <AdminBox>
          <>
            <div className="adm--box-fixed">
              <div className="adm--box-tit">{`SHOP 관리 ${viewId == null ? '등록' : '수정'}`}</div>
                ...
                <div className="input-table-row-td">
                  <div className="input-table-row-td-img">
                    {view?.new_img_file ? (
                      ...
                      <img src={view?.org_file_path} alt="SHOP 이미지" />
                    ) : (
                      <div>이미지를 선택해주세요</div>
                    )}
                  </div>
