...
const AdminSubsDescPage = () => {
    ...
    const APIgetDescription = async () => {
        try {
            const result: any = await callAPI(HTTPMETHOD.GET, {}, '/api/subscriptions/font-guide-text', getAccessToken());
    ...
    const APIputDescription = async () => {
        ...
        try {
            ...
            const result: any = await callAPI(HTTPMETHOD.PUT, param, '/api/admin/font-subscription/guide-text', getAccessToken());
            return true;
        } catch (error) {
            console.log(error);
            throw error;
    ...
    return (
        <AdmWrapper>
            <AdmPageTop title={`폰트 구독 안내문구 관리`} />
            <AdmListBox>
                <>
