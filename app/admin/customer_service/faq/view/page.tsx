...
const API_URL_GET = '/api/admin/cs/faq/detail/';
const API_URL = '/api/admin/cs/faq';
const BOARD_URL = '/admin/customer_service/faq';

const AdminCustomerServiceFaqViewPage: React.FC = () => {
    ...
    const callView = async () => {
        ...
        try {
            ...
            updateValue('order', Number(result.data.order), setViewTableObject);
            updateValue('title', result.data.title || '', setViewTableObject);
            updateValue('content', result.data.content || '', setViewTableObject);
            updateValue('categoryId', Number(result.data.category.id) || '', setViewTableObject);
            // updateValue('categoryId', 15, setViewTableObject);
    ...
    useEffect(() => {
        APIgetCategoryList().then((res: any) => {
            ...
            const categoryOptions = res.map((item: any) => ({
                value: Number(item.id),
                label: item.name,
            }));

            setViewTableObject(prev => prev.map(item =>
                item.inputKey === 'categoryId'
                    ? { ...item, selectOptions: categoryOptions }
                    : item
            ));
        })
    }, [])
    // #endregion category get ========================================
    return (
        <AdmWrapper>
            <AdmPageTop title={`FAQ ${viewId == null ? '등록' : '수정'}하기`} depth1={'FAQ 관리'} depth2={`FAQ ${viewId == null ? '등록' : '수정'}하기`} />
            <AdmViewBox>
                <>
                    <AdmViewMakeTable
...
export default AdminCustomerServiceFaqViewPage;