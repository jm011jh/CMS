...
const SettingChatroomPopup: React.FC<SettingChatroomPopupProps> = ({ close }) => {
    ...
    const [screenFileUrl, setScreenFileUrl] = useState<string>('');
    const handleScreenFile = () => {
        ...
        fileInput.onchange = (e) => {
            ...
            if (file) {
                if (screenFileUrl) {
                    URL.revokeObjectURL(screenFileUrl);
                }
                const newUrl = URL.createObjectURL(file);
                setScreenFile(file);
                setScreenFileUrl(newUrl);
            }
        };
        fileInput.click();
    }
                                ...
                                <div style={styles.screenPreviewImg}>
                                    {
                                        screenFile
                                            ? <>
                                                <div style={styles.screenPreviewImgFile}></div>
                                                <HeicThumbnail
                                                    fileUrl={screenFileUrl}
                                            ...
                                            screenImageUrl
                                                ? <><div style={styles.screenPreviewImgFile}></div>
                                                    <HeicThumbnail
                                                        fileUrl={screenImageUrl}
                                                        fileName={screenImageUrl}
                                                        sizes="100%"
                                                        objectFit={'cover'}
                                                    />
                                                </>
