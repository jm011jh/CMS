...
type AdmSubsUserPopupProps = {
    ...
    openMore?: any;
    openAddSubs?: any;
    openAddSubsStart?: any;
}

const AdmUserSubscriptionList: React.FC<AdmSubsUserPopupProps> = ({ data, status, statusChange, openMore, openAddSubs, openAddSubsStart }) => {
    ...
    const tableStructureData: AdmListTableCellProps[] = [
        { id: '', value: '사용자 ID', size: '180px', cellKey: 'name' },
        { id: '', value: '플랫폼', size: '100px', cellKey: 'email' },
        { id: '', value: '구독 상품명', size: '200px', cellKey: 'currentStatus' },
        { id: '', value: '주문 ID', size: '100%', cellKey: 'totalSubscriptionMonths' },
        { id: '', value: '구독시작일', size: '180px', cellKey: 'storeType' },
        // { id: '', value: '최근결제일', size: '180px', cellKey: 'productName' },
        { id: '', value: '만료일', size: '180px', cellKey: 'productName' },
        { id: '', value: '다음결제일(자동갱신시)', size: '180px', cellKey: 'productName' },
        { id: '', value: '구독상태', size: '100px', cellKey: 'productName' },
                    ...
                    <div style={styles.body}>
                        <div style={styles.bodyBox}>
                            {
                                ...
                                //             <div style={styles.bodyBoxInfoTableValue}>{detailData.plan.isActive ? '활성' : '해지'}</div>
                                //         </div>
                                //         <div style={styles.width50}>
                                //             <div style={styles.bodyBoxInfoTableLabel}>누적구독횟수</div>
                                //             <div style={styles.bodyBoxInfoTableValue}>{detailData.totalSubscriptionMonths}</div>
                                //         </div>
                                //     </div>
                                //     <div style={styles.width50}>
                                //         <div style={styles.bodyBoxInfoTableLabel}>만료일</div>
                                //         <div style={styles.bodyBoxInfoTableValue}>{dayjs(detailData.subscriptionPeriods[0].endDate).format('YYYY.MM.DD HH:mm')}</div>
                                //     </div>
                                // </div>
                            }
                        </div>
                        <div style={styles.bodyBox}>
                            <div style={styles.bodyBoxHead}>
                                <div style={styles.bodyBoxHeadLeft}>
                                    <div style={styles.bodyBoxHeadTitle}>구독 내역</div>
                                    <button style={styles.bodyBoxHeadMoreButton} onClick={openAddSubsStart}>구독 시작일 강제 설정</button>
                                    <button style={styles.bodyBoxHeadMoreButton} onClick={openAddSubs}>구독 종료일 강제 설정</button>
                                </div>
                                <div style={styles.bodyBoxHeadRight}>
                                    {
                                        detailData &&
                                        <button style={styles.bodyBoxHeadMoreButton} onClick={openMore}>구독내용 더보기</button>
                                    }
                                ...
                                </div>
                                {
                                    detailData && detailData.subscriptions && detailData.subscriptions.map((period: any, index: number) => (
                                        <div key={index} style={{
                                            ...
                                            <div style={{ ...styles.bodyBoxScrollTableValue, width: '180px', minWidth: '180px', borderLeft: '1px solid #E5E7EB' }}>{detailData.user.name}</div>
                                            <div style={{ ...styles.bodyBoxScrollTableValue, width: '100px', minWidth: '100px', borderLeft: `1px solid ${basicThemeColors.gray300}` }}>{period.storeType === 'PLAY_STORE' ? 'GOOGLE' : 'Apple'}</div>
                                            <div style={{ ...styles.bodyBoxScrollTableValue, width: '200px', minWidth: '200px', borderLeft: `1px solid ${basicThemeColors.gray300}` }}>{period.plan.name || '-'}</div>
                                            <div style={{ ...styles.bodyBoxScrollTableValue, width: '100%', minWidth: '240px', borderLeft: `1px solid ${basicThemeColors.gray300}` }}>{period.originalTransactionId}</div>
                                            <div style={{ ...styles.bodyBoxScrollTableValue, width: '180px', minWidth: '180px', borderLeft: `1px solid ${basicThemeColors.gray300}` }}>{dayjs(period.startDate).format('YYYY.MM.DD HH:mm')}</div>
                                            {/* <div style={{ ...styles.bodyBoxScrollTableValue, width: '180px', minWidth: '180px', borderLeft: `1px solid ${basicThemeColors.gray300}` }}>{dayjs(period.lastPaymentDate).format('YYYY.MM.DD HH:mm')}</div> */}
                                            <div style={{ ...styles.bodyBoxScrollTableValue, width: '180px', minWidth: '180px', borderLeft: `1px solid ${basicThemeColors.gray300}` }}>{dayjs(period.endDate).format('YYYY.MM.DD HH:mm')}</div>
                                            <div style={{ ...styles.bodyBoxScrollTableValue, width: '180px', minWidth: '180px', borderLeft: `1px solid ${basicThemeColors.gray300}` }}>{
                                                period.nextPaymentDate
                                                    ? dayjs(period.nextPaymentDate).format('YYYY.MM.DD HH:mm')
                                                    : '-'
                                            }</div>
                                            <div style={{ ...styles.bodyBoxScrollTableValue, width: '100px', minWidth: '100px', borderLeft: `1px solid ${basicThemeColors.gray300}` }}>
...
export default AdmUserSubscriptionList;